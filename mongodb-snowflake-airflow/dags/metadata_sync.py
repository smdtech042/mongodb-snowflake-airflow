...
def metadata_sync_dag():
    ...
    def setup_metadata_table(**context):
        ...
        logger.info(f"Metadata sync log table setup completed.")
    ...
    def sync_audit_stats(**context):
        ...
        for col, mapping in SCHEMA_MAPPINGS.items():
            ...
            try:
                ...
                logger.error(f"Error fetching count for Mongo collection '{col}': {str(e)}")
                mongo_count = -1
            ...
            try:
                ...
                logger.error(f"Error fetching count for Snowflake table '{table}': {str(e)}")
                sf_count = -1
        ...
        if insert_values:
            ...
            logger.info(f"Successfully recorded metadata sync logs for {len(insert_values)} collections.")
        else:
